/* 02-06-22 updated: USING FOR 3-2 PROJECT
 ...
 * pin config: GND-GND; VCC-3.3V; SCL-B6; SDA-B7;
...
void general_menu(void){
   	  while(count<=5){
   	  	  ...
   	  	  if(count==5){
   	  	  SSD1306_GotoXY(0,51);
   	  	  SSD1306_Puts("5.how to use OLED",&Font_7x10,1);
   	  	  HAL_Delay(500);
   	  	  }
...
int main(void)
{
  ...
  SSD1306_Init();
      ...
      SSD1306_UpdateScreen();
      HAL_Delay(1000);
      SSD1306_Puts ("project for 3-2", &Font_11x18, 1);
      SSD1306_UpdateScreen();
      HAL_Delay (1000);
...
static void MX_GPIO_Init(void)
{
  ...
  //reseting
  HAL_GPIO_WritePin(GPIOA, 0B00001000010000, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(GPIOB, 0B00000000000001, GPIO_PIN_RESET);


  //for A: // A4 is output
  GPIO_InitStruct.Pin = 0B00000000010000;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  //for B: A0 will work for interrupt
  GPIO_InitStruct.Pin = 0B00001000000000;      //**** for interrupt
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);


  /* EXTI interrupt init*/            // **** for interrupt
